...
const StarSVG = ({ id, checked }: StarButton) => {
  const [checkedStar, setChecked] = useState(checked);
  const onClick = useCallback(
    (event: React.MouseEvent) => {
      event?.stopPropagation();
      storageHelper(id);
      setChecked(!checkedStar);
    },
    [checkedStar, id]
  );
  useEffect(() => {}, [checkedStar, id]);
  return (
    <div className="star">
      <svg
      ...
      >
        <path
          ...
          stroke={checkedStar ? "#5E96FC" : "#ACADB9"}
          strokeWidth="1.5"
        />
      </svg>
    </div>
